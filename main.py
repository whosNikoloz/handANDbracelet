...
def tflite_detect_camera(model_path, label_map_path, min_conf=0.8):
    ...
    while True:
        ...
        # Variables to store the bounding boxes of the bracelet and hand
        bracelet_box = None
        hand_box = None
        ...
        for i in range(len(scores)):
            if scores[i] > min_conf:
                ...
                cv2.putText(frame, label_text, (xmin, ymin - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

                # Check if "blue_bracelet" detected
                if label == 'blue_bracelet':
                    bracelet_detected = True
                    bracelet_box = (xmin, ymin, xmax, ymax)

                # Check if "custom_hand" detected
                if label == 'custom_hand':
                    hand_detected = True
                    hand_box = (xmin, ymin, xmax, ymax)

        # Check if both bracelet and hand are detected and if the bracelet is inside the hand
        if bracelet_detected and hand_detected:
            if (bracelet_box[0] > hand_box[0] and
                bracelet_box[1] > hand_box[1] and
                bracelet_box[2] < hand_box[2] and
                bracelet_box[3] < hand_box[3]):
                print("Blue bracelet detected inside custom hand!")
